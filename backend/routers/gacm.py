...
async def get_chat_history(db: AsyncSession = Depends(get_db)):
    """Fetches saved AI chat session history from PostgreSQL."""
    try:
...
async def save_chat_session(payload: dict, db: AsyncSession = Depends(get_db)):
    """Saves AI chat session history to PostgreSQL."""
    try:
        session_rec = GACMChatSession(
            user_id=DEFAULT_USER_ID,
            query_text=payload.get("query_text", ""),
            synthesized_answer=payload.get("synthesized_answer", "")[:1000],
